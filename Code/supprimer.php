<?php
// Vérifiez si l'ID a été envoyé via la méthode GET
if(isset($_GET['id'])) {
    ...
    // Requête de suppression du fichier dont l'ID est pensé en GET
    ...
    if ($pdo->query($requete) == TRUE) {
        ...
        if ($nombre_scans > 0) {
            ...
            header("Location: gestion_fichiers.php?alert=success&id_supprime=$id"); // Rediriger vers la page de gestion des fichiers
            exit();
        }
        // Cas où le nombre de scan est incorrect
        else {
            header("Location: gestion_fichiers.php?alert=error"); // Rediriger vers la page de gestion des fichiers avec une erreur
            exit();
        }
    }
}    
